...
def main():
    ...
    # Search queries (search API has 30/minute rate limit)
    print(f"\n[2] Searching {len(SEARCH_QUERIES)} queries (search API: 30/min limit)...")
    search_success = 0
    for i, q in enumerate(SEARCH_QUERIES):
        ...
        try:
            ...
            if isinstance(data, dict) and (data.get('message', '').lower().find('rate limit') >= 0 or data.get('documentation_url', '').find('rate-limit') >= 0):
                print(f" -> SEARCH RATE LIMIT REACHED (got {search_success} successful searches)")
                print(f"  Remaining repos from known games: {len(all_repos)}")
                break
            ...
            if 'rate limit' in err_str or '429' in err_str or '403' in err_str:
                print(f" -> SEARCH RATE LIMIT (got {search_success} successful searches)")
                print(f"  Falling back to {len(all_repos)} known repos")
                break
            print(f" Error: {e}")
            time.sleep(3)
